...
from PyQt5.QtCore import pyqtSignal, pyqtSlot, Qt, QRunnable, QThreadPool
import sys
import cv2
import imutils
import numpy as np
...
class Worker(QRunnable):
    ...
    def __init__(self, model_fcn, input_model, input_cam, input_lim, input_pts):
    ...
    def run(self):
        ...
        '''
        print("Thread start")
        self.model_fcn(self.input_model, self.input_cam, self.input_lim, self.input_pts)
        # time.sleep(5)
...
class Dialog(QDialog):
    ...
    def runModel(self):
        ...
        self.worker = Worker(self.AImodel_tpu, self.input_Model, self.input_Cam, self.input_Limit, two_points)
        self.threadpool.start(self.worker)

        #self.ai = AImodel_cpu("15fps.mp4", two_points)
        # self.ai.start()
        #self.p = Process(target=AImodel_cpu, args=("15fps.mp4", two_points,))
        # self.p.daemon = True
        # self.p.start()
        # self.p.join()
