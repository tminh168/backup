...
stream_3 = 'rtsp://192.168.200.80:555/user=admin_password=tlJwpbo6_channel=1_stream=0.sdp?real_stream'
model_1 = 'detection_1_edgetpu.tflite'
model_2 = 'detection_2_edgetpu.tflite'
model_3 = 'detection_1_edgetpu.tflite'
...
def tf_inference():
    ...
    interpreter_3 = tflite.Interpreter(model_3, experimental_delegates=[tflite.load_delegate('libedgetpu.so.1')])
    ...
    top_k = 5
    threshold = 0.8
    ct1 = CentroidTracker(maxDisappeared=2, maxDistance=65)
    ct2 = CentroidTracker(maxDisappeared=2, maxDistance=65)
    ct3 = CentroidTracker(maxDisappeared=2, maxDistance=65)
    trackableObjects1 = {}
    trackableObjects2 = {}
    trackableObjects3 = {}
    totalCount1 = 0
    ...
    fvs3 = WebcamVideoStream(src=stream_3).start()
    time.sleep(1.0)
    fps = FPS().start()
    ...
    while True:
        ...
        cv2_im3, totalCount3, direction_str3, trackableObjects3 = append_objs_to_img(cv2_im3, objs3, labels, ROI, ct3, trackableObjects3, totalCount3)
        ...
        info3 = [
           ("Direction", direction_str3),
           ("Count", totalCount3),
        ]
        for (i, (k, v)) in enumerate(info3):
           text3 = "{}: {}".format(k, v)
           cv2.putText(cv2_im3, text3, (10, H - ((i * 20) + 20)),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)

        if log_img:
          if direction_str1 == "In" or direction_str1 == "Out":
            cv2.imwrite("./detected1" + "/count-%d.jpg" % totalCount1, cv2_im1)
          if direction_str2 == "In" or direction_str2 == "Out":
            cv2.imwrite("./detected2" + "/count-%d.jpg" % totalCount2, cv2_im2)
          if direction_str3 == "In" or direction_str3 == "Out":
            cv2.imwrite("./detected3" + "/count-%d.jpg" % totalCount3, cv2_im3)

        cv2.imshow('Camera_1', cv2_im1)
        cv2.imshow('Camera_2', cv2_im2)
        cv2.imshow('Camera_3', cv2_im3)
        print('counting..')
        fps.update()
        end_t = time.time()
