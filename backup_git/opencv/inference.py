...
def tf_inference(stream, model, output):
    print('Loading model..')
    interpreter = tflite.Interpreter(model, experimental_delegates=[tflite.load_delegate('libedgetpu.so.1')])
    ...
    fvs = WebcamVideoStream(src=stream).start()
    time.sleep(1.0)
    fps = FPS().start()
    ...
    while True:
        ...
        if direction_str == "In" or direction_str == "Out":
           cv2.imwrite(output + "/count-%d.jpg" % totalCount, cv2_im)
        #if direction_str == '...':
        #   print('scanning...')
        #else: print('detected..!!!')
...
if __name__ == '__main__':
    p1 = Process(target=tf_inference(stream_1, model_1))
    p1.start()
    p2 = Process(target=tf_inference(stream_2, model_2))
    p2.start()
    p1.join()
    p2.join()
