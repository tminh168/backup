...
def tf_count():
    ...
    with open('counter1.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["Count", "Time", "Direction", "Filename"])
    with open('counter2.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["Count", "Time", "Direction", "Filename"])
    ...
    token = 'NKbqe8ovfMetW8WUrmVN7MtTDKsy6tCo6mm7ZX9Y'
    header = {'Authorization': '{}'.format(token)}
    url = 'https://beta.busmap.com.vn:2309/api/shift/count/people'
    q1 = Queue(maxsize=50)
    q2 = Queue(maxsize=50)
    dequeue = True
    ...
    while True:
        ...
        if log_img:
            if direction_str1 == "In" or direction_str1 == "Out":
                current_time1 = calendar.timegm(time.gmtime())
                filename1 = '{}_{}_{}.jpg'.format(
                    current_time1, direction_str1, totalCount1)
                dequeue = False
                cv2.imwrite(base_dir + "/detected1/" + filename1, cv2_im1)
                with open('counter1.csv', 'a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([totalCount1, current_time1, direction_str1, filename1])
                try:
                    with open(os.getcwd()+'/detected1/'+filename1, "rb") as image_file:
                        files = {'people_count': image_file}
                    ...
                    if r.status_code == 200:
                        if res["status"] == True:
                            os.remove(os.getcwd()+'/detected1/'+filename1)
                        else:
                            q1.put(filename1)
                    else:
                        q1.put(filename1)
            ...
            if direction_str2 == "In" or direction_str2 == "Out":
                current_time2 = calendar.timegm(time.gmtime())
                filename2 = '{}_{}_{}.jpg'.format(
                    current_time2, direction_str2, totalCount2)
                dequeue = False
                cv2.imwrite(base_dir + "/detected2/" + filename2, cv2_im2)
                with open('counter2.csv', 'a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([totalCount2, current_time2, direction_str2, filename2])
                try:
                    with open(os.getcwd()+'/detected2/'+filename2, "rb") as image_file:
                        files = {'people_count': image_file}
                        r = requests.post(url, headers=header,
                                          files=files, verify=False)
                    ...
                    if r.status_code == 200:
                        if res["status"] == True:
                            os.remove(os.getcwd()+'/detected2/'+filename2)
                        else:
                            q2.put(filename2)
                    else:
                        q2.put(filename2)
