...
def restart(totalCount1, totalCount2, model_1, model_2):
    total = str(str(totalCount1)+'\n'+str(totalCount2))
    with open("/home/mendel/coral/DFM_counter/count.txt", "w") as file:
       file.write(total)
    ...
    #with open("/home/mendel/coral/DFM_counter/count.txt", "r") as file:
    #   count_ckpt = file.read().split('\n')
    # int(count_ckpt[0]), int(count_ckpt[1])
    return DNN_count1, DNN_count2
...
def tf_count():
    ...
    te_init = time.time()
    print('init: {}'.format(te_init - t_init))
    ...
    while True:
        ...
        if int(te_dtc) >= timeout_ckpt:
            ...
            countedID1 = 0
            countedID2 = 0
            DNN_count1, DNN_count2 = restart(
                 totalCount1, totalCount2, model_1, model_2)
            timeout_ckpt = int(time.time()) + timeout_rs
            print('restart:{:.3f}'.format(time.time() - t_rs))
