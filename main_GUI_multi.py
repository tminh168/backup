...
class Dialog(QDialog):
    ...
    def runModel(self):
        ...
        if self.input_Cam == "192.168.200.81":
            ...
            fvs.release()
            two_points = mouse_pts
        elif self.input_Cam == "Both camera":
            input_cam1 = cam_81
            input_cam2 = cam_82
            fvs = cv2.VideoCapture("15fps.mp4")
            ret, frame = fvs.read()
            ...
            while ret:
                ...
                if len(mouse_pts) == 3:
                    ...
                    break
            fvs.release()
            two_points2 = mouse_pts

        if self.input_Cam == "Both camera":
            self.p1 = Process(target=AImodel_tpu, args=(
                self.input_Model, self.input_Cam, self.input_Limit, two_points1,))
            self.p2 = Process(target=AImodel_tpu, args=(
                self.input_Model, self.input_Cam, self.input_Limit, two_points2,))
            self.p1.start()
            self.p2.start()
            self.p1.join()
            self.p2.join()
